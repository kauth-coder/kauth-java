...
public class Main {
    public static void main(String[] args) {
        ...
        initConfigReq.setProgramId(10001L);
        // 设置程序密钥 (从后台获取: 程序管理 -> 程序列表)
        initConfigReq.setProgramSecret("your_program_secret");
        // 设置商户公钥 (从后台获取：系统设置 -> 密钥配置 -> RSA公钥 -> PKCS#8格式 )
        initConfigReq.setMerchantPublicKey("your_merchant_public_key");
        // 设置签名方式 (RSA)
        initConfigReq.setSignType(KauthSignEnums.SIGN_TYPE_RSA);
        // 初始化 SDK
        KauthCoreCore.init(initConfigReq);
        System.out.println("KAuth SDK 初始化完成");
    }
}
```
...
```java
 public static void main(String[] args) throws IOException {
    ...
    // 设置商户公钥 (从后台获取：系统设置 -> 密钥配置 -> RSA公钥 -> PKCS#8格式 )
    initConfigReq.setMerchantPublicKey("MIGfMA0GCSqGSIb3DQEBAQ......");
    // 设置签名方式 (RSA)
    initConfigReq.setSignType(KauthSignEnums.SIGN_TYPE_RSA);
    // 初始化 SDK
    KauthCoreCore.init(initConfigReq);
